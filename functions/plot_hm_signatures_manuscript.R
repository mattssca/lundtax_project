...
#'}
#'
plot_hm_signatures_manuscript = function(these_predictions = NULL,
                              this_data = NULL,
  ...
  #get subtype colors
  col = list(`7 Class` = lund_colors$lund_colors,
             `5 Class` = lund_colors$lund_colors)
  
  #predictions
  pred_lab_5 = pred_labels5
  
  split = factor(pred_lab_5, levels = c("Uro", "GU", "BaSq", "Mes", "ScNE"))
  ...
  if(subtype_annotation == "7_class"){
    ...
    #predictions
    pred_lab_7 = pred_labels7
    
    #overwrite predictions
    split = factor(pred_lab_7 ,levels = c("UroA" ,"UroB", "UroC", "GU", "BaSq", "Mes", "ScNE"))
  ...
  hm_a_predictions = HeatmapAnnotation(`5 Class` = pred_lab_5,
                                       `7 Class` = pred_lab_7,
  ...
  if(length(genes_early) != 0 & !all(is.na(genes_late))){
    ...
    #heatmap annotation
    col = list(`Late/Early Ratio` = col_fun_cc)
